...
for s = ilist
    ...
    ERODE = false; 
    if ERODE
        vmask = imerode(vmask, SE1); %#ok<*UNRCH>
        vmask = largestcomp(vmask);
    end
    dist = dist.*slicemask.*vmask;
    skel = bwskel(dist>0);
    dskel = skel.*dist;
    ...
    sortingCriteria = 3;
    CL = [];
    [CL.CL, CL.BM,CL.JM,~,~,CL.BL] ...
        = centerlineCSF(skel>0, spurLength, sortingCriteria);

    centerlineFile = fullfile(baseparent, 'CSFmasks', subjname, 'CL.mat');

    distMasked = dist.*vmask;
    dilCL = imdilate(CL.BM>0, SE1);
    distCL = dilCL.*distMasked;
    cubemasked = vmask.*cube;

    SHOWON = false;
    if SHOWON
        figure, sliceViewer(vmask);
        figure, imagesc(max(CL.BM, [], 3));
        figure, sliceViewer(vmask);
        figure, sliceViewer(distCL);
        figure, sliceViewer(distMasked);
        figure, sliceViewer(cubemasked);
    end

    save(centerlineFile, 'CL');
    ...
    % Compute mean waveform across bins
    meanWave = nanmean(bpc1, 2);
    
    % Compute error per bin (RMSE between each bin and mean waveform)
    rmse = sqrt(mean((bpc1 - meanWave).^2, 1, 'omitnan'));
    
    % Z-score RMSE across bins
    zrmse = zscore(rmse);
    
    % Threshold for outliers (adjust if needed)
    outlierThresh = 3; % bins >3 SD away from mean are outliers
    
    % Logical mask for valid (non-outlier) bins
