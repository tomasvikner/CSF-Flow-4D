...
% *** LOAD DATA ***
    function loadData()
        ...
        % TEMP: editing this to not overwrite CSF_GUI_rad3D output 
        % savefolder = fullfile(subjectFolder, 'Waveforms');
        savefolder = fullfile(subjectFolder, 'BoxWFs');
        if ~exist(savefolder, 'dir')
            mkdir(savefolder);
        end

        % TEMP: adding FS seg instead of distance for CA definition? 
        aafile = fullfile(subjectFolder, 'aa_nn4d.nii.gz');
        data.aa = MRIread(aafile).vol;
        data.aa = imrotate(data.aa, -90);

        BOXON = true;
        if BOXON == false % old solution 
            ...
            % data.roi = flip(roi, 2);
            data.roi = roi;
        else % box covering relevant ROIs 
            rvelsfolder = fullfile(subjectFolder, 'pca');
            load([rvelsfolder, '/BVELS.mat'], 'BVELS');
            data.vx = BVELS.BOX.vx;
            data.vy = BVELS.BOX.vy;
            data.vz = BVELS.BOX.vz;
            data.roi = BVELS.BOX.vol;
        end
        ...
        % TEMP: set VENTS to show instead of distance
