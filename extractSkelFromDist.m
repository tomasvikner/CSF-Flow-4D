...
for s = ilist
    ...
    rdskel = dskel .*slicemask;
    rdskel(rdskel>max(dists)) = 0;

    spurLength = 24; 
    sortingCriteria = 3; 
    [CL, branchMat,junctionMat,branchTextList,junctionList,branchList] ...
        = centerlineCSF(rdskel>0, spurLength, sortingCriteria);

    % figure, imagesc(max(branchMat, [],3)), colormap parula;
    distMat = (branchMat>0).*rdskel;
    % figure, imagesc(max(distMat, [],3)), colormap parula;

    distMatfile = fullfile(baseparent, 'CSFmasks', subjname, 'distMat.mat');
    save(distMatfile, 'distMat');
